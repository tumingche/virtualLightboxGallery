...
;(function($){
	...
	var defaults = {
		...
		easing: null,         //if using CSS: 'linear', 'ease', 'ease-in', 'ease-out', 'ease-in-out', 'cubic-bezier(n,n,n,n)'. If not using CSS: 'swing', 'linear'
		slideMargin: 0,       //Margin between each slide
		startSlide: 0,        //Starting slide index (zero-based)
		randomStart: false,   //Start slider on a random slide (boolean)
		...
		pagerSelector: null,  //Element used to populate the populate the pager. By default, the pager is appended to the bx-viewport
		buildPager: null,     //If supplied, function is called on every slide element, and the returned value is used as the pager item markup
		pagerCustom: null,    //Parent element to be used as the pager. Parent element must contain a  <a data-slide-index="x">  element for each slide
		...
		autoStart: true,      //Auto show starts playing on load. If  false , slideshow will start when the "Start" control is 
		autoDirection: 'next',//The direction of auto show slide transitions. 'next', 'prev'
		autoHover: false,     //Auto show will pause when mouse hovers over slider
		autoHoverPlay: false, //(CUSTOMIZED) Auto show will play when mouse hovers over slider
	...
	$.fn.bxSlider = function(options){
		...
		var start = function(){
			...
			// (CUSTOMIZED) if auto is false and has more than 1 page, start the show
			if (!slider.settings.auto && slider.settings.autoStart && (getPagerQty() > 1 || slider.settings.autoSlideForOnePage)) initAutoPlay();
		...
		*/
        ...
        //////////////////////////////////////////////////////////////////////////////////////
		/**
		 * (CUSTOMIZED) Initialzes the auto process
		*/
        var initAutoPlay = function(){
			// (CUSTOMIZED) if autoDelay was supplied, STOP the auto show using a setTimeout() call
			if(slider.settings.autoDelay > 0){
				var timeout = setTimeout(el.stopAuto, slider.settings.autoDelay);
			// (CUSTOMIZED) if autoDelay was not supplied, STOP the auto show normally
			}else{
				el.stopAuto();
			}
			// (CUSTOMIZED) if autoHoverPlay is requested
			if(slider.settings.autoHoverPlay){
				// on el hover
				el.hover(function(){
					// if the auto show is currently NOT playing (has NO active interval)
					if(!slider.interval){
						// START the auto show and pass true agument which will prevent control update
						el.startAuto(true);
						//el.stopAuto(false);
